// src/pages/ClassDetailsPage/AssignmentsTab.tsx

import React, { useState, useEffect } from 'react';
import { useParams, useOutletContext } from 'react-router-dom';
import * as api from '../../api/apiService';
// 🛑 SỬA LỖI 1: Import User và Assignment từ file types chung
...
/* 🛑 SỬA LỖI 2: Xóa interface Assignment cục bộ
interface Assignment { id: number; title: string; dueDate: string; }
*/

const AssignmentsTab: React.FC = () => {
  const { classId } = useParams<{ classId: string }>();
  // 🛑 SỬA LỖI 1 (tiếp): Dùng kiểu User import từ types
  ...
  const fetchAssignments = () => { 
    if(classId) {
      api.getAssignments(classId)
        // 🛑 SỬA LỖI 3: Đọc dữ liệu từ response.data.data
        .then(res => setAssignments(res.data.data))
        .catch(err => console.error(api.getErrorMessage(err)));
    }
  };
  ...
  const handleConfirmSubmission = async (file: File) => { 
    ...
    // TODO: Cần logic upload file thật sự
    // Tạm thời chỉ dùng file.name
    await api.submitAssignment(selectedAssignment.id, { fileUrl: file.name }); 
